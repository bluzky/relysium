;;; relysium-prompt-suggest.el --- Suggest prompts for relysium -*- lexical-binding: t; -*-
...
;; User prompt builder for suggest command
(defun relysium-prompt-suggest-user (context additional-prompt)
  "Build a suggest user prompt with CONTEXT and ADDITIONAL-PROMPT."
  (let* ((lang-name (plist-get context :language-name))
         ...
         (annotated-code (relysium-format-with-line-numbers buffer-content))
         (instruction (cond
                       ;; No specific prompt - provide a general improvement request
                       ((string-empty-p additional-prompt)
                        "Please analyze this code and suggest improvements focusing on:
1. Code clarity and organization
2. Error handling and edge cases
3. Performance optimizations where appropriate
4. Documentation improvements
Prioritize suggestions that would have the most positive impact.")
                       ;; User provided a specific prompt - use it with guidance
                       (t
                        (format "Please analyze this code and suggest improvements with these specific instructions:

%s

When suggesting changes:
- Make sure they address the specific requirements above
- Ensure all suggestions maintain the existing functionality unless explicitly requested otherwise
- Use a combination of small targeted changes and larger refactorings as appropriate"
                                additional-prompt)))))

    (relysium-build-prompt
     (list
      :a_file_info (format "File type: %s" lang-name)
      :b_code (format "Source code:\n%s"
                      (relysium-format-code-block lang-name annotated-code))
      :c_query instruction))))

(provide 'relysium-prompt-suggest)
;;; relysium-prompt-suggest.el ends here
